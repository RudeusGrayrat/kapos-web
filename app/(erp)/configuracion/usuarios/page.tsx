  ...
  function renderUserFields(target: "create" | "edit", values: UserForm, setValues: React.Dispatch<React.SetStateAction<UserForm>>) {
    ...
    return (
      <div className="grid gap-4 md:grid-cols-2">
        <label className="space-y-2"><span className="text-sm font-semibold text-[#0D0D0D]">Correo</span><input type="email" className={`w-full rounded-[20px] border border-[#E4E4E4] px-4 py-3 text-sm outline-none transition focus:border-[#00C70D] ${editingMembershipOnly ? "bg-[#f8faf2] text-[#6c755c]" : "bg-white"}`} value={values.email} onChange={(event) => setValues((current) => ({ ...current, email: event.target.value.toLowerCase() }))} disabled={editingMembershipOnly} required /></label>
        {target === "create" ? (
          <label className="space-y-2"><span className="text-sm font-semibold text-[#0D0D0D]">Contrasena temporal</span><input type="password" className="w-full rounded-[20px] border border-[#E4E4E4] bg-white px-4 py-3 text-sm outline-none transition focus:border-[#00C70D]" placeholder="Opcional: si queda vacio sera invitado" value={values.password} onChange={(event) => setValues((current) => ({ ...current, password: event.target.value }))} /></label>
        ) : (
          <label className="space-y-2"><span className="text-sm font-semibold text-[#0D0D0D]">Estado global</span><input className="w-full rounded-[20px] border border-[#E4E4E4] bg-[#f8faf2] px-4 py-3 text-sm text-[#6c755c] outline-none" value={values.userStatus} disabled /></label>
        )}
        <label className="space-y-2"><span className="text-sm font-semibold text-[#0D0D0D]">Nombres</span><input className={`w-full rounded-[20px] border border-[#E4E4E4] px-4 py-3 text-sm outline-none transition focus:border-[#00C70D] ${editingMembershipOnly ? "bg-[#f8faf2] text-[#6c755c]" : "bg-white"}`} value={values.firstName} onChange={(event) => setValues((current) => ({ ...current, firstName: event.target.value }))} disabled={editingMembershipOnly} /></label>
        <label className="space-y-2"><span className="text-sm font-semibold text-[#0D0D0D]">Apellidos</span><input className={`w-full rounded-[20px] border border-[#E4E4E4] px-4 py-3 text-sm outline-none transition focus:border-[#00C70D] ${editingMembershipOnly ? "bg-[#f8faf2] text-[#6c755c]" : "bg-white"}`} value={values.lastName} onChange={(event) => setValues((current) => ({ ...current, lastName: event.target.value }))} disabled={editingMembershipOnly} /></label>
        <label className="space-y-2"><span className="text-sm font-semibold text-[#0D0D0D]">Tipo documento</span><select className={`w-full rounded-[20px] border border-[#E4E4E4] px-4 py-3 text-sm outline-none transition focus:border-[#00C70D] ${editingMembershipOnly ? "bg-[#f8faf2] text-[#6c755c]" : "bg-white"}`} value={values.documentType} onChange={(event) => setValues((current) => ({ ...current, documentType: event.target.value as UserForm["documentType"] }))} disabled={editingMembershipOnly}><option value="DNI">DNI</option><option value="RUC">RUC</option><option value="CE">CE</option><option value="PASSPORT">Pasaporte</option></select></label>
        <label className="space-y-2"><span className="text-sm font-semibold text-[#0D0D0D]">Documento</span><input className={`w-full rounded-[20px] border border-[#E4E4E4] px-4 py-3 text-sm outline-none transition focus:border-[#00C70D] ${editingMembershipOnly ? "bg-[#f8faf2] text-[#6c755c]" : "bg-white"}`} value={values.documentNumber} onChange={(event) => setValues((current) => ({ ...current, documentNumber: event.target.value }))} disabled={editingMembershipOnly} /></label>
        <label className="space-y-2"><span className="text-sm font-semibold text-[#0D0D0D]">Telefono</span><input className={`w-full rounded-[20px] border border-[#E4E4E4] px-4 py-3 text-sm outline-none transition focus:border-[#00C70D] ${editingMembershipOnly ? "bg-[#f8faf2] text-[#6c755c]" : "bg-white"}`} value={values.phone} onChange={(event) => setValues((current) => ({ ...current, phone: event.target.value }))} disabled={editingMembershipOnly} /></label>
        <label className="space-y-2"><span className="text-sm font-semibold text-[#0D0D0D]">Cargo</span><input className="w-full rounded-[20px] border border-[#E4E4E4] bg-white px-4 py-3 text-sm outline-none transition focus:border-[#00C70D]" placeholder="Cajero, supervisor..." value={values.title} onChange={(event) => setValues((current) => ({ ...current, title: event.target.value }))} /></label>
        <label className="space-y-2"><span className="text-sm font-semibold text-[#0D0D0D]">Codigo interno</span><input className="w-full rounded-[20px] border border-[#E4E4E4] bg-white px-4 py-3 text-sm outline-none transition focus:border-[#00C70D]" value={values.employeeCode} onChange={(event) => setValues((current) => ({ ...current, employeeCode: event.target.value }))} /></label>
        <label className="space-y-2"><span className="text-sm font-semibold text-[#0D0D0D]">Estado en organizacion</span><select className="w-full rounded-[20px] border border-[#E4E4E4] bg-white px-4 py-3 text-sm outline-none transition focus:border-[#00C70D]" value={values.membershipStatus} onChange={(event) => setValues((current) => ({ ...current, membershipStatus: event.target.value as UserForm["membershipStatus"] }))}><option value="ACTIVE">Activo</option><option value="INVITED">Invitado</option><option value="SUSPENDED">Suspendido</option><option value="INACTIVE">Inactivo</option><option value="TERMINATED">Terminado</option></select></label>
      </div>
    );
  }
      ...
      {mode === "create" ? (
        ...
        <PanelCard title="Tabla de usuarios" description="Accesos internos de la organizacion activa.">
          <AdminDataTable
            ...
            columns={[
              { key: "user", label: "Usuario", render: (row) => <div><p className="font-semibold text-[#0D0D0D]">{userDisplayName(row)}</p><p className="text-xs text-[#A1A1A1]">{row.user.email ?? row.user.documentNumber ?? "sin identificador"}</p></div> },
