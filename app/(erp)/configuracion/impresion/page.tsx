  ...
  return (
    <section className="space-y-8">
      <AdminModuleHeader
        ...
        action={<Settings2 className="h-6 w-6 text-[#00C70D]" />}
        stats={[
      ...
      >
        <div className="grid gap-3">
          {printRoutes.map((route) => (
            <article key={route.name} className="rounded-[22px] border border-[#e7edd9] bg-[#F8F8F8] p-4">
              <div className="flex flex-wrap items-start justify-between gap-3">
                <div>
                  <h3 className="text-base font-semibold text-[#0D0D0D]">{route.name}</h3>
      ...
      <PanelCard
        ...
        action={<ReceiptText className="h-5 w-5 text-[#00C70D]" />}
      >
      ...
      <PanelCard
        ...
        action={<Utensils className="h-5 w-5 text-[#00C70D]" />}
      >
